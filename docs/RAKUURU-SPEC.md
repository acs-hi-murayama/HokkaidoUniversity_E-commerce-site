...
らくうるカートのショップは **2つのドメイン** に分かれます。ここを取り違えると全部ずれます。
...
| **テーマ** | 店舗ごとの見た目一式（テンプレHTML群＋CSS＋デザイン設定）。このショップのテーマIDは JS パス `cms/yt00037/` に現れる |
| **デザイン設定** | 管理画面でトップの各ブロックの表示ON/OFFや見出し文言を決める設定。テンプレでは `dsnTop*` 変数として現れる |

---

## 1. ストアフロントの組み立て方（Common_Part 方式）
...
### 差し込みプレースホルダ（`<[-- … --]>`）
`Common_Part.html` にある **4つの差し込み口**。`{{ }}` ではなく `<[-- … --]>` という独自記法で、
サーバーがビルド時に該当HTMLへ置換します。
...
### 2-6. 一覧のページング・並び替え（Product_List.html）
| 変数 | 意味 |
|---|---|
...
## 3. Twig 制御構文の読み方（テンプレを読む最低限）

原本を読むために必要な構文はこれだけです。
...
> **ローカルの命名差**：本番は `/item-list`・`/item-detail/{id}`。ローカルは `serve` の都合で
> `product-list?categoryId=`・`product-details?id=`（拡張子なし）にしている。移行時はここを本番名に戻す。

### 4-2. 取引アプリ（cart.raku-uru.jp）— `/{機能}/{段階}/{shpHash}`
...
> `yt00037` がこのショップの**テーマID**。JS/画像のパスに現れる。

---

## 7. 管理画面で決まること vs テンプレで決まること

「どこを触れば変わるか」を、本番運用の観点で切り分けます。

| 変えたいもの | 本番での操作場所 | テンプレ上の現れ方 |
|---|---|---|
| ショップ名・ロゴ・ファビコン | 管理画面 基本設定 | `shopName` `hdrLogoImageUrl` `topFaviconUrl` |
| トップの各ブロックON/OFF・見出し | 管理画面 デザイン設定 | `dsnTopDispFlag*` `dsnTopTitle*` |
| キービジュアル・フリー欄の中身 | 管理画面 デザイン設定（HTML入力） | `dsnTopHtmlTag` `dsnTopDesc1/2Shop` |
| 商品（名前/価格/画像/在庫/バリエーション） | 管理画面 商品管理 | `itemList` / `itemData.*` |
| カテゴリー・キャンペーン | 管理画面 カテゴリ/キャンペーン設定 | `categoryList` `campaignList` |
| お知らせ（フリーページ） | 管理画面 フリーページ | `freepageList` → `/fr/{no}` |
| 取引画面の色 | 管理画面 デザイン設定（テーマ色） | `getCss/{shpHash}` が再生成 |
| 会員機能・お問合せの有無 | 管理画面 機能設定 | `registValidFlag` `newMemRegistFlag` `askpageUseFlag` |
| ページ共通のHTML構造（ヘッダー等） | **テーマHTMLの編集**（Common_Part 等） | テンプレ直接 |
| 商品ページのレイアウト | テーマHTMLの編集（Product_*.html） | テンプレ直接 |
...
## 8. このローカル環境との対応（移行の地図）

本ローカル環境は「らくうるカートの外枠（HTML/CSS/URL）はそのまま、動的処理だけJS/localStorageで仮実装」した状態です。

| らくうるカート本番 | ローカル再現 | 本番へ戻すとき |
|---|---|---|
| `Common_Part.html`＋`<[-- MAIN_CONTENTS --]>` 注入 | 各HTMLに共通枠を直書き | 共通枠を1つに集約し MAIN_CONTENTS 部分だけ各ページに |
| `{% for item in itemList %}`（SSR） | `js/shop.js`＋`products.json` | JSを撤去しTwigループへ |
| `{{ dsnTopDesc1Shop }}` 等 | `data/*.html` を `Shop.include()` | 管理画面のデザイン設定へ貼付 |
| `{{ cssAddr }}` / `getCss` | `css/*.css` / `cart/assets/theme.css` | テーマCSS・管理画面デザイン設定へ |
| `/incart` POST・セッション/DB | `cart/assets/cart-app.js`＋localStorage | サーバー処理（撤去） |
| `/item-detail/{id}`・`/item-list?…` | `product-details?id=`・`product-list?…` | 本番URL名に戻す |

機械変換は **`build-production` スキル**（[.claude/skills/build-production/SKILL.md](../.claude/skills/build-production/SKILL.md)）が
`production-build/` に出力します。手動で仕上げる項目は `production-build/MIGRATION-NOTES.md` に列挙されます。
